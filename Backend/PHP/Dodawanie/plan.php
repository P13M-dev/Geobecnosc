...
$db = Database::get();
$wynik = $db->query("SELECT COUNT(lekcje.id) as istnieje FROM lekcje WHERE dzien = ?",[$plan[0]["data"]]);
$klasa = $_POST["klasa"];
if($wynik[0]["istnieje"]){ // nadpisz lekcje
    foreach($plan as $dzien){
        foreach($dzien["plan"] as $lekcja){
    ...
    foreach($plan as $dzien){
        foreach($dzien["plan"] as $lekcja){
            ...
            foreach($uczniowie as $uczen){
                $db->query("INSERT INTO obecnosci_na_lekcji (lekcja,uczen) VALUES (?,?)",[$id_lekcji,$uczen["id"]]);
            }
        }
    }
}
